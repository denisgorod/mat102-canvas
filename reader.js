...
// Data + graph
// ---------------------------------------------------------------------------
const dataRes = await fetch("./reader-data.json");
if (!dataRes.ok) throw new Error(`Failed to load reader-data.json (${dataRes.status}). Run tools/build-reader-data.py and commit the result.`);
...
// Session state: a Review pass over due exercises (self-rated).
let session = null;   // { items:[{kind:'exercise',slug}], pos, review }
let preSession = null;
...
style.textContent = `
  ...
  /* NOTE: collides with <aside class="rd-next">, which relies on this. */
  .rd-next { border-color:var(--obs-accent); color:var(--obs-accent); font-weight:600; }
...
// Bottom section for a bit or exercise.
...
function startReview() {
  ...
  const items = due.map((slug) => ({ kind: "exercise", slug }));
  preSession = current; session = { items, pos: 0, review: true };
  enterItem();
}
function enterItem() {
  current = currentItem().slug;
